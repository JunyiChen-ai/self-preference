#!/usr/bin/env python3
"""Plot selection ratios for the multi-model comparison experiment."""
...
def list_evaluators(root: Path) -> list[str]:
    evaluator_dirs = sorted([p.name for p in root.iterdir() if p.is_dir()])
    if not evaluator_dirs:
        raise SystemExit(f"No evaluator directories found under {root}")
    return evaluator_dirs


def gather_counts(root: Path, evaluators: list[str]):
...
def arrange_generators(evaluators: list[str], generators: set[str]) -> list[str]:
    ordered = []
    seen = set()
    for name in evaluators:
        can = canonical(name)
...
def plot_heatmap(ratios: np.ndarray, evaluators: list[str], generators: list[str], label: str, output: Path):
    fig, ax = plt.subplots(figsize=(10, 6))
    row_labels = [format_model_label(name) for name in generators]
    col_labels = [format_model_label(name) for name in evaluators]
    ...
    ax.set_title(f"Multi-Model Preference Matrix ({label})")
    fig.tight_layout()
...
def parse_args():
    parser = argparse.ArgumentParser(description="Plot results for the 9 LLM + human comparison task")
    parser.add_argument("dataset", help="Name of the dataset folder under the data directory (e.g., paper)")
    parser.add_argument(
        "--data-root",
        default="data",
    ...
    parser.add_argument(
        "--subdir",
        default="comparison_multi/comparison_9",
        help="Subdirectory inside the dataset folder that holds model outputs",
    )
    parser.add_argument(
        "--output",
        default="",
        help="Path for the generated plot. Defaults to plots/comparison_multi_<dataset>.png",
    )
...
def main():
    ...
    output = Path(args.output) if args.output else Path("plots") / f"comparison_multi_{args.dataset}.png"
    label = dataset_label(args.dataset)

    evaluators = list_evaluators(root)
    counters, totals, generators = gather_counts(root, evaluators)
    generator_order = arrange_generators(evaluators, generators)
    ratios = build_ratio_matrix(counters, totals, generator_order)
    plot_heatmap(ratios, evaluators, generator_order, label, output)
