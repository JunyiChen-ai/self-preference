...
# Unified color palette for bars
...
def gather_scores(root: Path, model_dirs: list[str]):
    totals = []
    wins = []
    ...
    for col, name in enumerate(model_dirs):
        model_dir = root / name
        ...
        for file_path in files:
            try:
                ...
                continue
            comp = canonical(data.get("comparison_model"))
            if comp != name:
                continue
...
def plot_bar(root: Path, output: Path, label: str, model_dirs: list[str]):
    wins, totals, _ = gather_scores(root, model_dirs)
    ...
    ax.set_title(f"Qwen3 Triple Comparison ({label})")
...
def plot_matrix(root: Path, output: Path, label: str, model_dirs: list[str]):
    _, totals, source_counts = gather_scores(root, model_dirs)
