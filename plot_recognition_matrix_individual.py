#!/usr/bin/env python3
"""Plot recognition-individual confusion matrix using is_yes ratio."""
...
DATASET_PATHS: Dict[str, Path] = {
    "paper": Path("self-preference/data/paper/recognition_individual"),
    "news": Path("self-preference/data/news_segment/recognition_individual"),
    "news_segment": Path("self-preference/data/news_segment/recognition_individual"),
}
        ...
        for row, source in enumerate(sources):
            ...
            for file_path in files:
                try:
                    ...
                    continue
                if bool(data.get("is_yes")):
                    yes_count += 1
            matrix[row, col] = yes_count / total if total else 0.0
    return matrix
...
def plot_heatmap(matrix: np.ndarray, evaluators: List[str], sources: List[str], label: str, output: Path):
    ...
    ax.set_title(f"Self-Recognition Individual Heatmap ({label})")
    fig.tight_layout()
...
def parse_args():
    parser = argparse.ArgumentParser(description="Plot recognition_individual confusion matrix")
    parser.add_argument("dataset", help="Dataset name (e.g., paper or news)")
    parser.add_argument(
        "--input-dir",
        default="",
        help="Override input directory; otherwise dataset must be known",
    )
    parser.add_argument(
        "--output",
        default="",
        help="Output path; defaults to self-preference/plots/indi_recognition_<dataset>.png",
    )
...
def main():
    ...
    output = Path(args.output) if args.output else Path("self-preference/plots") / f"indi_recognition_{args.dataset}.png"
