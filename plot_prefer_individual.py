#!/usr/bin/env python3
"""Plot individual preference confusion matrix for provided dataset."""
...
DATASET_PATHS: Dict[str, Path] = {
    "paper": Path("self-preference/data/paper/prefer_individual"),
    "news": Path("self-preference/data/news_segment/prefer_individual"),
    "news_segment": Path("self-preference/data/news_segment/prefer_individual"),
}
...
def discover_axes(root: Path) -> Tuple[List[str], List[str]]:
    ...
    for evaluator in evaluators:
        ...
        for sub in subdirs:
            if sub not in seen:
                sources.append(sub)
                seen.add(sub)
    # ensure evaluators appear first in the source order
    ordered_sources = []
    added = set()
    for name in evaluators:
        if name in seen and name not in added:
            ...
            added.add(name)
    for name in sources:
        if name not in added and name != "human":
            ordered_sources.append(name)
            added.add(name)
        ...
        for row, source in enumerate(sources):
            ...
            for file_path in files:
                try:
                    ...
                    continue
                if str(data.get("preference", "")).lower() == "like":
                    likes += 1
            matrix[row, col] = likes / total if total else 0.0
    return matrix
...
def plot_heatmap(matrix: np.ndarray, evaluators: List[str], sources: List[str], label: str, output: Path):
    ...
    sns.heatmap(
        ...
        cbar_kws={"label": "Preference Ratio"},
        ax=ax,
        linewidths=0.5,
        linecolor="white",
        square=False,
    ...
    ax.set_title(f"Individual Preference Heatmap ({label})")
    fig.tight_layout()
...
def parse_args():
    parser = argparse.ArgumentParser(description="Plot preference confusion matrix")
    parser.add_argument("dataset", help="Dataset name (e.g., paper or news)")
    parser.add_argument(
        "--input-dir",
        default="",
        help="Override input directory; if not set, dataset name must be in the predefined map",
    )
    parser.add_argument(
        "--output",
        default="",
        help="Optional output path; defaults to plots/indi_prefer_<dataset>.png",
    )
...
def main():
    ...
    if args.input_dir:
        ...
        if key not in DATASET_PATHS:
            raise SystemExit(
                f"Unknown dataset '{args.dataset}'. Provide --input-dir or use one of: {', '.join(DATASET_PATHS)}"
            )
    ...
    output = Path(args.output) if args.output else Path("plots") / f"indi_prefer_{args.dataset}.png"
